...
public class Player4 : MonoBehaviour
{
    ...
    void Update()
    {
        if (!championSelect.PlayerJoined[playerNumber] && GamePad.GetButton(CButton.Start, PlayerIndex.Four))
        {
            championSelect.PlayerJoined[playerNumber] = true;
            SelectionSquare.SetActive(true);
            print("PlayerFour joined");
        }

        var horizontal = GamePad.GetAxis(CAxis.LX, PlayerIndex.Four);
        ...
        if (GamePad.GetButton(CButton.A, PlayerIndex.Four) && !playerValider && SelectionSquare.activeInHierarchy)
        {
        ...
        if (GamePad.GetButton(CButton.B, PlayerIndex.Four))
        {
