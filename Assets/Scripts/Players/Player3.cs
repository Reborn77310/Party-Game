...
public class Player3 : MonoBehaviour
{
    ...
    void Update()
    {
        if (!championSelect.PlayerJoined[playerNumber] && GamePad.GetButton(CButton.Start, PlayerIndex.Three))
        {
            championSelect.PlayerJoined[playerNumber] = true;
            SelectionSquare.SetActive(true);
            print("PlayerThree joined");
        }

        var horizontal = GamePad.GetAxis(CAxis.LX, PlayerIndex.Three);
        ...
        if (GamePad.GetButton(CButton.A, PlayerIndex.Three) && !playerValider && SelectionSquare.activeInHierarchy)
        {
            if (!championSelect.SelectedPlayers[playerNumber].activeInHierarchy &&
                !championSelect.isPlayerSelected[actualPlayerSelected])
            {
                GameMaster.PlayerThree = actualPlayerSelected;
